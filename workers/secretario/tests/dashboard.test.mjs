...
test('generated browser JavaScript parses and exposes Aravitas sales dashboard',async()=>{
 ...
 assert.ok(html.includes('Molécula o presentación'));
 assert.ok(html.includes('sales-product-evolution'));
 assert.ok(html.includes('sales-product-inactive'));
 assert.ok(html.includes('sales-product-sort'));
 assert.ok(html.includes('product-list'));
 ...
 assert.ok(script.includes('Buscar Brand'));
 assert.ok(script.includes('Buscar molécula, presentación o código'));
 ...
 assert.ok(script.includes('No hay Brands que coincidan con estos filtros'));
 assert.ok(script.includes('Tendencia último mes'));
 assert.ok(script.includes('Tendencia 3 meses'));
 assert.ok(script.includes('Total periodo'));
 assert.ok(script.includes('Unidades · mayor a menor'));
 assert.ok(script.includes('Todas las moléculas y presentaciones'));
 assert.ok(script.includes('enableSalesLayout'));
 assert.ok(script.includes('aravitas-dashboard-layout-'));
 assert.ok(source.includes("content:'✓ Comprado · '"));
