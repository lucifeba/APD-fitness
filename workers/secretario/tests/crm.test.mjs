...
const js=ts.transpileModule(source,{compilerOptions:{module:ts.ModuleKind.ES2022}}).outputText.replace(/^import .*;$/gm,'');
const {validateCrm,crmApi,crmFields}=await import('data:text/javascript;base64,'+Buffer.from(js).toString('base64'));
...
test('visit records retain calendar location, classification and route',()=>{
 assert.ok(crmFields.visits.includes('Dirección'));
 assert.ok(crmFields.visits.includes('Clasificación'));
 assert.ok(crmFields.visits.includes('Ruta'));
});
